...
flags.DEFINE_list("dims", [784, 1000, 1000, 10], "Dimensions about layers including output")
...
if FLAGS.load_from is not None:
	path = os.path.join('/data/veniat/lileb/datasets',
						FLAGS.load_from,
						'mnist-T{tid}_{split}.pth'
																	)
	...
	for task in range(10):
		...
		for k, v in Xs.items():
			...
			y.scatter_(1, loaded_y, 1)
			v.append((loaded_x.numpy(), y.numpy()))
else:
...
for t in range(FLAGS.n_classes):
	if FLAGS.load_from is not None:
		data = (*Xs['train'][t], *Xs['val'][t], *Xs['test'][t])
	else:
